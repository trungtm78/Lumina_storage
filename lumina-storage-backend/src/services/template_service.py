...
_FILL_TOOLS = Path(__file__).resolve().parent.parent.parent / "skills" / "docx-form-fill" / "tools"
...
def _extract_segments(doc) -> list[dict]:
    ...
    """
    import sys
    if str(_FILL_TOOLS) not in sys.path:
        sys.path.insert(0, str(_FILL_TOOLS))
    import importlib.util
    spec = importlib.util.spec_from_file_location("fill", _FILL_TOOLS / "fill.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod._extract_segments(doc)
            ...
            for ri, row in enumerate(table.rows):
                ...
                for ci in range(len(row.cells)):
                    ...
                    if not cell_segs:
                        ...
                        row_parts.append(f'<cell id="{cell_base}">{inner}</cell>')
...
Với `seg`: dùng đúng giá trị thuộc tính `id` của thẻ `<p>` hoặc `<cell>` chứa trường cần điền.
...
async def commit_template(
    ...
    for field in normalized:
        seg = seg_by_loc.get(field["location"])
        if not seg:
            logger.warning("Field %s references unknown location %s", field["name"], field["location"])
            continue
        para = seg["para_ref"]
